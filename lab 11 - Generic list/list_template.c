...
{
	ListElement *node = p_list->head;

	while (node != NULL)
	{
		p_list->dump_data(node->data);
		node = node->next;
	}
}
...
void dump_list_if(List *p_list, void *data)
{
	...
	while (node != NULL)
	{
		if (p_list->compare_data(node->data, data) == 0)
			p_list->dump_data(node->data);
...
void insert_elem(List *p_list, void *p_data)
{
	...
	else
	{
		...
		while (first != NULL && p_list->compare_data(p_data, first->data) >= 0)
		{
			prev = first;
			first = first->next;
		}

		// Check if element is repeated in list
