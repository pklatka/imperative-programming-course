...
void bernoulli_gen(int v[], int bernoulli_percentage, int n)
{
    ...
    {
        int random = rand() % 100;
        if (random < bernoulli_percentage)
        {
            v[i] = 1;
        }
        else
...
void pmf(double v[], int trials)
{
    for (int i = 0; i < trials; i++)
    {
        ...
        int throw_sum = (rand() % 6 + 1) + (rand() % 6 + 1);
        v[throw_sum - 2] += 1;
    }

    double multiply = 100.0 / trials;
...
void monty_hall(int *wins, double *win_probability, int n)
{
    *wins = 0;
    for (int i = 0; i < n; i++)
    {
        ...
        int remaining = 3 - reveal - choice; // your second choice (if you switch)
        if (prize == remaining)
            ++(*wins); // you win if you switched
    }
